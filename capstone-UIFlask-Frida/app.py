...
playstore=playstore.astype({'Reviews':'int','Size':'int','Installs':'int'})
...
def index():
    ...
    priceavgbox=df2.groupby(by='Category').agg({'Price' : 'mean' }).rename({'Price':'Average_Price'}, axis=1).sort_values(by='Average_Price',ascending=False).head().reset_index()
    X=priceavgbox.Category
    Y=priceavgbox.Average_Price
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.bar(X,Y,color=['purple'])
    plt.xlabel('Category')
    plt.ylabel('Average_Price')
    plt.savefig('priceavgbox.png',bbox_inches="tight")
